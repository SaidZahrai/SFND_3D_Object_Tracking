...
void matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource, std::vector<cv::KeyPoint> &kPtsRef, cv::Mat &descSource, cv::Mat &descRef,
                      std::vector<cv::DMatch> &matches, std::string descriptorOutputType, std::string matcherType, std::string selectorType,
                      double &t)
{
    // configure matcher
    ...
    {
        int normType = descriptorOutputType.compare("DES_BINARY") == 0 ? cv::NORM_HAMMING : cv::NORM_L2;
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType, double &t)
{
    ...
    {
        std::cout << "MP.4 - Unknown keypoint descriptor: " << descriptorType << ".\n";
    }
...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, double &t, bool bVis)
{
    ...
    cv::goodFeaturesToTrack(img, corners, maxCorners, qualityLevel, minDistance, cv::Mat(), blockSize, useShiTomasi, k);
...
void detKeypointsHarris(vector<cv::KeyPoint> &keypoints, cv::Mat &img, double &t, bool bVis)
{
    // compute detector parameters based on image size
    int blockSize = 4;       //  size of an average block for computing a derivative covariation matrix over each pixel neighborhood
    double maxOverlap = 0.0; // max. permissible overlap between two features in %
...
// Detect keypoints in image using other detectors
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, double &t, bool bVis)
{
    ...
    else if (detectorType.compare("AKAZE") == 0)
    {
        ...
        detector = cv::AKAZE::create(descriptor_type, descriptor_size, descriptor_channels, threshold, nOctaves, 
                                    nOctaveLayers = 4, diffusivity);
    }
    ...
    {
        cout << "ERRPR:: Unknown detector type: " << detectorType << ". No detection performed-" << endl;
        return;
    }
